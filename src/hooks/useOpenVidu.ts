...
export const useOpenVidu = ({ sessionId, userName }: UseOpenViduParams) => {
  ...
  const normalizeToken = (rawToken: string): string => {
    try {
      ...
      // 프로토콜 강제 wss + 포트 제거
      url.protocol = 'wss:';
      url.port = ''; // remove :4443

      return url.toString(); // token=tok_... 포함 전체 URL 반환
    } catch (error) {
  ...
  const joinSession = async () => {
    ...
    try {
      ...
      const rawToken = tokenRes.data; // raw: ws://...:4443?sessionId=...&token=tok_...
      const token = normalizeToken(rawToken); // token: wss://.../sessionId=...&token=tok_...
